...
void display(void) {
    ...
    gluLookAt(0.0, 0.0, 10*RATIO, 0.0, 0.0, 0.0, 0.0, 7.0, 0.0);
...
//*PLY光点
   ...
   glEnd();
    ...
    for (int i = 0; i < 10; i++) {
        for (int j = 0; j < 9; j++) {
            glVertex2f(3*O[i][j][0], 3*O[i][j][1]);
            glVertex2f(3*O[i][j+1][0], 3*O[i][j+1][1]);
        }
    }
    glEnd();
    ...
    for (int i = 0; i < 10; i++) {
        for (int j = 0; j < 19; j++) {
            glVertex2f(-7.0+3 * O[i][j][0], 3 * O[i][j][1]);
            glVertex2f(-7.0+3 * O[i][j + 1][0], 3 * O[i][j + 1][1]);
        }
        glVertex2f(-7.0 + 3 * O[i][19][0], 3 * O[i][19][1]);
        glVertex2f(-7.0 + 3 * O[i][0][0], 3 * O[i][0][1]);
    }
    glEnd();
    // */
/*
...
lbfgsfloatval_t sampling(lbfgsfloatval_t E, lbfgsfloatval_t Beta, lbfgsfloatval_t* en) {
    ...
    while (1) {
        while (1) {
            z1 = -1.0 + 2.0 * (double)rand() / RAND_MAX;
            z2 = -1.0 + 2.0 * (double)rand() / RAND_MAX;
        ...
        E = z1;
        Beta = z2;
        u = -1.0 + 2.0 * ((double)rand() / RAND_MAX);
        //printf("%f %f ", z1, z2);
...
int main(int argc, char **argv)
{
    ...
    int COUNT = 0;
    for (int i = 0; i < 10; i++) {
        o[i][0] = 5.0 + cos(i * pi / 5) * 0.75;
        o[i][1] = 5.0  + sin(i * pi / 5) *0.75;
        for (int j = 0; j < 20; j++) {
            O[i][j][0] = o[i][0] + 0.27 * cos(j * pi / 10-(5 - 2*i) * pi / 10);
            O[i][j][1] = o[i][1] + 0.27 * sin(j * pi / 10-(5 - 2*i) * pi / 10);
